...
class site_router implements base_interface_router {
    ...
    private function parse_query($query) {
        ...
        if (count($matchs[0])) {
            return array('query' => $query, 'module' => $matchs[1][0], 'extension' => $extended_name, 'separator' => $matchs[2][0]);
        }

        preg_match_all('/^([^.]+)$/isU', $query, $matchs);
        if (count($matchs[0])) {
            return array('query' => $query, 'module' => $matchs[1][0], 'extension' => $extended_name, 'separator' => $this->get_separator());
        }
