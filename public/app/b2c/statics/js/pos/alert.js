...
;(function(){
	...
	var jqueryAlert = function(opts){
		...
		var $modal = jq("<div class='alert-modal none'>");
		var $container = jq("<div class='alert-container animated'>");
		var $title = jq("<div class='alert-title'>"+option.title+"</div>");
		var $content = jq("<div class='alert-content'>");
		var $buttonBox = jq("<div class='alert-btn-box'>");
		var $closeBtn = jq("<div class='alert-btn-close'>×</div>");
		var $actionsheetCloseBtn = jq("<p class='alert-btn-p alert-btn-sheet'>"+ option.actionsheetCloseText +"</p>"); 
		...
		dialog.framework = function(){
			...
			if(option.modal || option.modal == 'true'){
				...
				option.bodyScroll && jq('body').css('overflow','hidden');
			};
			...
			if(option.position == 'absolute'){
				$container.css({
					'position' : option.position,
					'left'     : jq(window).width()/2,
					'top'      : jq(window).height()/2 + jq(window).scrollTop(),
					'z-index'  : zIndex,
				});
			};
			...
			if(parseInt(jq(window).height()) <=  parseInt($container.css('height'))){
				$container.css('height',jq(window).height());
			};
			...
			for(var key in option.buttons){
				indexs++;
				var $button = jq("<p class='alert-btn-p alert-btn-p"+indexs+"'>"+ key +"</p>");
				if(option.style == 'wap'){
					$button.css({
			...
			if(option.actionsheetCloseText){
				...
				jq(document).delegate('.alert-btn-sheet,.alert-modal','click',function(){
					dialog.close();
				})
		...
		function isSelfClose(){
			if(dialog.buttonsLength <= 0 && option.style == 'wap'){
				setTimeout(function(){
					...
					option.bodyScroll && jq('body').css('overflow','auto');
		...
		dialog.show = function(){
			...
			$modal.fadeIn(dialog.time);
			option.bodyScroll && option.modal && jq('body').css('overflow','hidden');
			isSelfClose();
		};
