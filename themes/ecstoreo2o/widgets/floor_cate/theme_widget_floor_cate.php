...
function theme_widget_floor_cate(&$setting,&$render){
    ...
    $salesList = _ex_vertical_getSales2();
    ...
    foreach ($cat_list as $k=>$cat) {
        switch ($cat['step']) {
        case 1:
            $all_cids 	= _ex_vertical_getAllChildAttr2($cat_list,$cat['cat_id']);
            $all_cids[] 	= $cat['cat_id'];
            $all_typeids 	= _ex_vertical_getAllChildAttr2($cat_list,$cat['cat_id'],'type');
            $all_typeids[] 	= $cat['type'];
            $all_brandids 	= _ex_vertical_getLinkBrandIds2($all_typeids);
...
function _ex_vertical_getAllChildAttr2($arr,$pid,$attribute = 'cat_id'){
    foreach ($arr as $item) {
...
function _ex_vertical_getLinkBrandIds2($typeids){
...
function _ex_vertical_getSales2(){
